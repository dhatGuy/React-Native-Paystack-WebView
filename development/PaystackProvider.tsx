...
import { generatePaystackParams, handlePaystackMessage, paystackHtmlContent, validateParams } from './utils';
...
export const PaystackProvider: React.FC<PaystackProviderProps> = ({
  ...
  const [method, setMethod] = useState<'checkout' | 'newTransaction' | 'resumeTransaction'>('checkout');

  const fallbackRef = useMemo(() => `ref_${Date.now()}`, []);

  const open = useCallback(
    (params: PaystackParams, selectedMethod: 'checkout' | 'newTransaction' | 'resumeTransaction') => {
  ...
  const resumeTransaction = (params: PaystackParams) => open(params, 'resumeTransaction');
